  ...
  return (
    ...
    <div className="h-full flex flex-col space-y-4 animate-in fade-in pb-10">
                        ...
                        {app ? (
                          <div className={`h-full w-full rounded-2xl border flex flex-col justify-between transition-all group ${app.status === 'CONCLUIDO_PAGO' ? 'border-emerald-500/40 bg-emerald-500/10' : app.status === 'NAO_COMPARECEU' ? 'border-red-500/40 bg-red-500/10' : app.awaitingOnlinePayment ? 'border-blue-400/50 bg-blue-500/10' : 'border-[#C58A4A]/30 bg-[#C58A4A]/5'} ${compactView ? 'p-1.5 rounded-lg' : 'p-2'}`}>
                            <div className="truncate" onClick={(e) => { e.stopPropagation(); setShowDetailModal(app); }} style={{cursor:'pointer'}}>
                              <div className="flex items-center gap-1">
                                <h4 className={`font-black uppercase truncate hover:text-[#C58A4A] transition-colors ${compactView ? 'text-[8px]' : 'text-[10px]'} ${app.status === 'NAO_COMPARECEU' ? 'text-purple-400' : theme === 'light' ? 'text-zinc-900' : 'text-white'}`}
                                  title="Ver detalhes"
                                >{app.clientName}</h4>
                            ...
                            <div className={`flex items-center justify-end gap-1 ${compactView ? 'mt-0.5' : 'mt-1'}`}>
                               ...
                               {app.status !== 'CONCLUIDO_PAGO' && app.status !== 'NAO_COMPARECEU' && app.status !== 'CANCELADO' && (
                                 <button onClick={(e) => { e.stopPropagation(); handleNoShowCancel(app); }} className={`rounded-lg transition-all bg-red-500/10 text-purple-400 hover:bg-red-500/30 ${compactView ? 'p-0.5' : 'p-1'}`} title="🚫 Cancelar por ausência — Lista Negra"><UserX size={compactView ? 9 : 11}/></button>
                               )}
                    ...
                    {(waitQueueStore || []).filter((w:any) => w.status === 'AGUARDANDO').map((w:any, idx:number) => (
                      <div key={w.id} className={`rounded-2xl border p-4 flex items-center gap-4 ${isDark ? 'bg-blue-500/5 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
                        ...
                        <div className="flex gap-2 shrink-0">
                          ...
                          <button onClick={() => removeFromWaitQueue(w.id)} className={`p-2 rounded-xl ${isDark ? 'bg-white/5 text-zinc-500 hover:text-purple-400' : 'bg-zinc-100 text-zinc-400 hover:text-red-500'} transition-all`}>✕</button>
                        </div>
                      </div>
                    ))}
                  </div>
                        ...
                        <div key={w.id} className={`rounded-2xl border p-4 flex items-center gap-4 ${isDark ? 'bg-blue-500/5 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
                          ...
                          <div className="flex gap-2 shrink-0">
                            ...
                            <button
                              ...
                              className={`p-2 rounded-xl ${isDark ? 'bg-white/5 text-zinc-500 hover:text-purple-400' : 'bg-zinc-100 text-zinc-400 hover:text-red-500'} transition-all`}
                            >
                              ✕
                            </button>
                          </div>
             ...
             {appointmentsFiltered.map(app => (
               <div key={app.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-[#C58A4A]/30 transition-all">
                  ...
                  <div className="flex items-center gap-2">
                     ...
                     {app.status !== 'CONCLUIDO_PAGO' && app.status !== 'NAO_COMPARECEU' && app.status !== 'CANCELADO' && (
                       <button onClick={() => handleNoShowCancel(app)} className="p-2 bg-red-500/10 border border-red-500/20 text-purple-400 hover:bg-red-500/20 rounded-xl transition-all" title="🚫 Cancelar por ausência — Lista Negra"><UserX size={16}/></button>
                     )}
               ...
               <div className="space-y-4">
                  ...
                  {newApp.clientId && (
                    <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${isDark ? 'bg-emerald-500/10 border border-emerald-500/20' : 'bg-emerald-50 border border-emerald-200'}`}>
                      ...
                      <button type="button" onClick={() => { setNewApp({...newApp, clientId: ''}); setClientSearch(''); }} className="text-zinc-500 hover:text-purple-400 text-[10px]">✕</button>
                    </div>
                  )}
      ...
      {showDetailModal && (() => {
        ...
        const statusColor = app.status === 'CONCLUIDO_PAGO' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30' : app.status === 'CANCELADO' ? 'text-purple-400 bg-red-500/10 border-red-500/30' : app.status === 'NAO_COMPARECEU' ? 'text-purple-400 bg-red-500/10 border-red-500/30' : app.awaitingOnlinePayment ? 'text-blue-400 bg-blue-500/10 border-blue-400/30' : 'text-[#C58A4A] bg-[#C58A4A]/10 border-[#C58A4A]/30';
        return (
                        ...
                        {photos.map((url: string, i: number) => (
                          <div key={i} className="relative group aspect-square rounded-2xl overflow-hidden border border-white/10">
                            ...
                            <button
                              ...
                              className="absolute top-1.5 right-1.5 bg-black/70 text-purple-400 rounded-xl p-1.5 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 hover:text-white"
                            >
                              <Trash2 size={10}/>
                            </button>
                          </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                {app.status !== 'CONCLUIDO_PAGO' && app.status !== 'NAO_COMPARECEU' && (
                  <button
                    ...
                    className="w-full py-3 rounded-xl font-black uppercase text-[9px] flex items-center justify-center gap-2 bg-red-500/10 border border-red-500/30 text-purple-400 hover:bg-red-500/20 transition-all"
                  >
                    🚫 Não Compareceu
                  </button>
                )}
                {app.status === 'NAO_COMPARECEU' && (
                  <div className="w-full py-3 rounded-xl text-center text-[9px] font-black uppercase bg-red-500/10 border border-red-500/30 text-purple-400">
                    ...
                    <div className="space-y-2">
                      ...
                      {finAdditionals.map(item => (
                        <div key={item.id} className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-white/3 border border-white/5' : 'bg-zinc-50 border border-zinc-100'}`}>
                          ...
                          <button onClick={() => setFinAdditionals(prev => prev.filter(a => a.id !== item.id))} className="text-purple-400 hover:text-red-500 text-xs">✕</button>
                        </div>
                      ))}
                    </div>
                  )}
                      ...
                      <div className={`p-4 rounded-2xl border-2 ${available ? 'border-[#C58A4A]/50 bg-[#C58A4A]/5' : 'border-red-500/30 bg-red-500/5'}`}>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          {available
                            ...
                            : <span className="text-[8px] font-black text-purple-400 bg-red-500/10 px-2 py-0.5 rounded-full">LIMITE ATINGIDO</span>
                          }
                        </div>
                        {maxCuts > 0 && (
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`text-[9px] font-black ${cutsUsed >= maxCuts ? 'text-purple-400' : 'text-[#C58A4A]'}`}>{cutsUsed}/{maxCuts}</span>
                          </div>
                        )}
                        <button type="button"
