      ...
      {products && products.some((p: any) => p.stock !== null && p.stock !== undefined && p.minStock !== null && p.minStock !== undefined && p.stock <= p.minStock) && (
        <div className={`rounded-2xl p-4 border flex items-start gap-3 ${isDark ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-200'}`}>
          ...
          <div className="flex-1">
            <p className="text-[10px] font-black uppercase tracking-widest text-purple-400 mb-2">⚠️ Estoque Baixo — Produtos precisam de reposição</p>
              ...
              <div className="p-6 space-y-3">
                ...
                {p.stock !== null && p.stock !== undefined && (
                  <div className={`rounded-xl p-3 border ${p.stock === 0 ? (isDark ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-200') : p.stock <= (p.minStock || 0) ? (isDark ? 'bg-amber-500/10 border-amber-500/20' : 'bg-amber-50 border-amber-200') : (isDark ? 'bg-white/5 border-white/10' : 'bg-zinc-50 border-zinc-200')}`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className={`text-[9px] font-black uppercase tracking-widest flex items-center gap-1 ${p.stock === 0 ? 'text-purple-400' : p.stock <= (p.minStock || 0) ? 'text-amber-400' : isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>
                        <ShoppingCart size={10}/> Estoque
                      </span>
                      <span className={`text-sm font-black ${p.stock === 0 ? 'text-purple-400' : p.stock <= (p.minStock || 0) ? 'text-amber-400' : isDark ? 'text-white' : 'text-zinc-900'}`}>
                        {p.stock} {p.stock === 0 ? '— ESGOTADO' : p.stock <= (p.minStock || 0) ? '— BAIXO' : 'un.'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => adjustStock(p.id, -1)} disabled={p.stock === 0} className="w-7 h-7 rounded-lg bg-red-500/20 text-purple-400 hover:bg-red-500/30 font-black text-sm flex items-center justify-center disabled:opacity-30">−</button>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button onClick={async () => await updateProduct(p.id, { active: p.active === false })}
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all ${p.active !== false ? (isDark ? 'bg-red-500/10 text-purple-400 hover:bg-red-500/20' : 'bg-red-50 text-purple-400') : (isDark ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-50 text-emerald-500')}`}>
                    {p.active !== false ? <><ToggleRight size={13}/> Ocultar</> : <><ToggleLeft size={13}/> Mostrar</>}
                  </button>
                  <button onClick={() => { if (window.confirm(`Remover "${p.name}"?`)) deleteProduct(p.id); }} className="p-3 rounded-xl bg-red-500/10 text-purple-400 hover:bg-red-500/20 transition-all">
                    <Trash2 size={14}/>
                  </button>
                </div>
              </div>
