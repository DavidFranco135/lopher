  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className={`text-3xl md:text-4xl font-black font-display tracking-tight flex items-center gap-3 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
            Gestão <span className="text-[#8B1F1C] italic">LOPHER</span>
          </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Hoje', value: revenue.daily, color: '#8B1F1C', sub: 'Faturamento diário' },
          ...
          <div
            ...
            className={`${cardClass} p-6 rounded-[2rem] cursor-pointer group hover:border-[#8B1F1C]/40 transition-all duration-300 relative overflow-hidden`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          ...
          { label: 'Novos Clientes/Mês', value: newClientsMonth, icon: Users, color: '#8B1F1C' },
        ].map((item) => (
          <div
            key={item.label}
            onClick={() => onNavigate('appointments')}
            className={`${cardClass} p-5 rounded-2xl cursor-pointer hover:border-[#8B1F1C]/30 transition-all group`}
          >
          ...
          <div className="space-y-3 max-h-[260px] overflow-y-auto scrollbar-hide">
            {todayApps.map(app => (
              <div
                key={app.id}
                onClick={() => onNavigate('appointments')}
                className={`flex items-center gap-3 p-3 rounded-2xl cursor-pointer transition-all group border border-transparent hover:border-[#8B1F1C]/20 ${isDark ? 'hover:bg-white/5' : 'hover:bg-zinc-50'}`}
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-sm transition-all group-hover:bg-[#8B1F1C] group-hover:text-black ${isDark ? 'bg-zinc-900 text-[#8B1F1C]' : 'bg-zinc-100 text-[#8B1F1C]'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={`${cardClass} rounded-[2rem] p-6 md:p-8`}>
          <div className="flex items-center gap-3 mb-6">
            <Trophy size={18} className="text-[#8B1F1C]" />
            ...
            <div className="space-y-3">
              {serviceRanking.map((svc, i) => (
                <div key={svc.name} className="flex items-center gap-4">
                  <span className={`text-[10px] font-black w-5 ${i === 0 ? 'text-[#8B1F1C]' : subTextClass}`}>#{i + 1}</span>
        ...
        <div className={`${cardClass} rounded-[2rem] p-6 md:p-8`}>
          <div className="flex items-center gap-3 mb-6">
            <Zap size={18} className="text-[#8B1F1C]" />
